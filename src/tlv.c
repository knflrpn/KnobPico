...
bool tlv_write(uint8_t* blob, uint16_t cap, uint16_t* woff, uint16_t type,
  ...
  if (!blob || !woff) return false;
  if ((uint32_t)(*woff) + sizeof(BkTlvHeader) + len > cap) return false;
...
bool tlv_next(const uint8_t* blob, uint16_t total_len,
  ...
  if (!blob || !roff || !out_hdr || !out_val) return false;
  if (*roff + sizeof(BkTlvHeader) > total_len) return false;

  memcpy(out_hdr, &blob[*roff], sizeof(BkTlvHeader));
  uint16_t val_off = (uint16_t)(*roff + sizeof(BkTlvHeader));

  if ((uint32_t)val_off + out_hdr->len > total_len) return false;
