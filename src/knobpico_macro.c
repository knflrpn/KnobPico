...
void macro_start(uint8_t macro_id) {
  ...
  if (macro_id == 0 || macro_id >= MAX_MACROS) return;

  const BkMacroStep* first = &st->cfg.macros[macro_id][0];
  if (first->duration_ms == 0 && first->keycode == 0 && first->modifiers == 0) return;
...
void macro_process(void) {
  KnobPicoState* st = knobpico_state();
  if (!st->macro_playing) return;

  uint32_t now = millis();
  if (now < st->macro_next_time_ms) return;
  ...
  if (step.keycode) key_array[0] = step.keycode;
