...
uint16_t knobpico_config_serialize(const KnobPicoConfig* cfg, uint8_t* blob, uint16_t cap) {
  ...
  if (cap < sizeof(BkConfigHeader)) return 0;
  ...
  for (uint8_t mid = 0; mid < MAX_MACROS; mid++) {
    ...
    for (int s = 0; s < MAX_MACRO_STEPS; s++) {
      ...
      if (st->duration_ms == 0 && st->keycode == 0 && st->modifiers == 0) break;
      count++;
    }
    if (count == 0) continue;
...
bool knobpico_config_apply(KnobPicoConfig* cfg,
                          ...
                          uint16_t max_len,
                          uint32_t now_ms) {
  if (!cfg || !blob) return false;
  if (max_len < sizeof(BkConfigHeader)) return false;
  ...
  if (hdr.total_len > max_len || hdr.total_len < sizeof(BkConfigHeader)) return false;
  ...
  while (tlv_next(blob, hdr.total_len, &roff, &tlv, &val)) {
    switch ((BkTlvType)tlv.type) {
      ...
      case TLV_MACRO_DEF: {
        ...
        if (mid >= MAX_MACROS || count > MAX_MACRO_STEPS || tlv.len < required) break;
